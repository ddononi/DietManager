...
public class DbHelper extends SQLiteOpenHelper {
	 ...
	 * @return
	 */
	public int insertGoalCal(int goalCal, CharSequence date) {
		int result;
		SQLiteDatabase db = this.getWritableDatabase();
	...
	public ArrayList<DayCalData> selectDayCalories() {
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = db.rawQuery(
				"SELECT * FROM cal_day_list ORDER BY date DESC", null);
		...
		if (cursor.moveToFirst()) {
			...
			do {
				...
				String date = cursor.getString(cursor.getColumnIndex("date"));
				int breakfast = cursor.getInt(cursor
						.getColumnIndex("breakfast"));
	...
	public ArrayList<PathPoint> selectExercusePath(int index) {
		...
		Cursor cursor = db
				.rawQuery(
						"SELECT * FROM trace_coord WHERE running_record_idx = ? ORDER BY idx ASC ",
		...
		if (cursor.moveToFirst()) {
			...
			do {
				...
				String dateMillis = cursor.getString(cursor
						.getColumnIndex("dateMillis"));
